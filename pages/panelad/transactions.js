...
export default function Transactions(props) {
  ...
  useEffect(() => {
    ...
    fetchUsers().then(() => console.log("done loading users"));
    filterByDate();
  }, [User]);
  ...
  const handleFilter = (type = "", assets = "") => {
    ...
    if (type == "filterByTransaction") {
      if (assets.target.value == -1) {
        ...
        if (coinSelected == -1 && userSelected == -1) {
          newData = data;
        }
      }
      if (coinSelected != -1) {
      ...
      if (userSelected != -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) =>
                x.user_info.username == userSelected &&
                x.t_type == assets.target.value
            ))
          : (newData = data.filter(
              (x) => x.user_info.username == userSelected
            ));
      }
      if (userSelected != -1 && coinSelected != -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) =>
                x.t_type == assets.target.value &&
    ...
    if (type == "filterByCoin") {
      ...
      if (assets.target.value == -1) {
        setCoinSelected(assets.target.value);
        if (transactionSelected == -1 && userSelected == -1) {
          newData = data;
        }
      }
      ...
      if (userSelected != -1 && transactionSelected != -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) =>
                x.t_type == transactionSelected &&
      ...
      if (userSelected == -1 && transactionSelected == -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) => x.coinData.symbol == assets.target.value
            ))
          : (newData = data);
      }
    }
    if (type == "filterByUsername") {
      // console.log('condition entered with value ',assets.target.value);
      setUserSelected(assets.target.value);
      if (assets.target.value == -1) {
        setUserSelected("All Assets");
        if (coinSelected == -1 && transactionSelected == -1) {
          newData = data;
        }
      }
      if (transactionSelected != -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) =>
                x.t_type == transactionSelected &&
                x.user_info.username == assets.target.value
            ))
          : (newData = data.filter((x) => x.t_type == transactionSelected));
      }
      if (coinSelected != -1) {
      ...
      if (transactionSelected != -1 && coinSelected != -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) =>
                x.t_type == transactionSelected &&
      ...
      if (transactionSelected == -1 && coinSelected == -1) {
        assets.target.value != -1
          ? (newData = data.filter(
              (x) => x.user_info.username == assets.target.value
            ))
          : (newData = data);
      }
    }
    if(transactionSelected == -1 && coinSelected == -1 && userSelected == -1){
      newData = data;
    }
    console.log(newData);
    setFiltered(newData);
                ...
                <div className="Filter">
                  <div className="card">
                    <div className="header mb-l">
                      <h4>Filters</h4>
                      <button className="Button primary" onClick={resetFilter}>Reset</button>
                    </div>
                    <fieldset>
                    ...
                    <fieldset>
                      ...
                      <div className="SelectBox filter">
                        <span>
                          <label>
                            {userSelected === -1 ? "All Assets" : userSelected}
                          </label>
                          <i className="fa fa-chevron-down" />
                          <select
                            onChange={(e) =>
                              handleFilter("filterByUsername", e)
                            }
                            id="filterByUsername"
                          >
